    ...
    try:
        ...
        if 'Date Required' in df.columns:
            ...
            try:
                 ...
                 # Add more formats if needed: format='%m/%d/%Y', etc.
            except Exception: # Fallback if specific formats fail
...
with tab1:
    ...
    if last_dept and last_dept in DEPARTMENTS:
        ...
        except ValueError: dept_index = 0 # Should not happen if list is consistent
    # Department selector
    dept = st.selectbox(
        "Select Department",
        DEPARTMENTS,
    ...
    for i in range(st.session_state.item_count):
        ...
        with st.expander(label=f"Item {i}: {item_label}", expanded=False):
            ...
            with col1:
                ...
                st.selectbox(
                    label=f"Item Select {i}", # Unique label for screen readers etc.
                    options=[""] + master_item_names, # Use full item list (no filtering)
