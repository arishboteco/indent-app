...
from fpdf import FPDF # Assuming fpdf2 is installed as 'fpdf' or 'fpdf2'
...
def create_indent_pdf(data: Dict[str, Any]) -> bytes:
    ...
    # FPDF's output(dest='S') returns bytes when using fpdf2, or string for older fpdf.
    # The error "'bytearray' object has no attribute 'encode'" implies output() is returning bytearray.
    # If using fpdf2 (which is in requirements.txt), output(dest='S') returns bytes.
    # st.download_button data parameter accepts bytes directly.
    pdf_output = pdf.output(dest='S')
    if isinstance(pdf_output, str): # Should not happen with fpdf2's 'S'
        return pdf_output.encode('latin-1')
    return pdf_output # Directly return bytes or bytearray
...
with tab1:
    ...
    items_to_render_list = list(st.session_state.form_items) 
    ...
    if st.button("Submit Indent Request", type="primary", use_container_width=True, disabled=submit_disabled_check, help=tooltip_msg_val):
        ...
        for item_dict_submit in st.session_state.form_items:
            ...
            if selected_item_submit and qty_submit > 0 and unit_submit != '-': 
            ...
            elif selected_item_submit and qty_submit > 0 and unit_submit == '-': 
        ...
        try:
            ...
            rows_to_add_gsheet = [
                [mrn_val_submit, timestamp_submit, requester_submit, dept_submit, formatted_date_submit,
                 item, f"{qty:.3f}", unit, note if note else "N/A"] 
    ...
    if st.session_state.get('submitted_data_for_summary'):
        ...
        st.markdown(f"**Total Submitted Items (sum of quantities):** {total_submitted_qty_val:.3f}"); st.divider() 
        ...
        with col_btn2: 
            try:
                ...
                st.link_button("✅ Prepare WhatsApp Message", wa_url_link, use_container_width=True) # Removed target argument
...
with tab2:
    ...
    log_df_tab2_val = load_indent_log_data() 
