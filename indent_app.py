...
try:
    ...
    else: creds_dict = json_creds_data
    ...
    try:
        ...
        st.error("Google Spreadsheet 'Indent Log' not found.")
        st.stop()
    except gspread.exceptions.WorksheetNotFound:
        st.error("Worksheet 'Sheet1' or 'reference' not found.")
        st.stop()
    except gspread.exceptions.APIError as e:
         st.error(f"Google API Error accessing sheets: {e}")
         st.stop()
...
def get_reference_data(_client):
    # ... (get_reference_data function remains the same - loads data, stores in state) ...
    try:
        ...
        for i, row in enumerate(all_data):
            ...
            if len(row) >= 2:
                item = str(row[0]).strip(); unit = str(row[1]).strip()
        ...
        st.error(f"Google Sheets API Error loading reference data: {e}"); return [], {}
    except Exception as e:
        st.error(f"Unexpected error loading reference data: {e}"); st.exception(e); return [], {}
...
def generate_mrn():
    ...
    except gspread.exceptions.APIError as e: st.error(f"API Error generating MRN: {e}"); return f"MRN-ERR-{datetime.now().strftime('%H%M%S')}"
    except Exception as e: st.error(f"Error generating MRN: {e}"); return f"MRN-FALLBACK-{datetime.now().strftime('%Y%m%d%H%M')}"
...
def update_unit_display(index):
    ...
    else: st.session_state[unit_display_key] = "-"
...
# --- Item Input Rows (NO st.form, WITH dropdown filtering) ---
# NOTE: Dynamic filtering of options *can* sometimes cause widgets to reset state on rerun.
# If resets persist, removing the filtering (using the full list) is the most stable option.
for i in range(st.session_state.item_count):

    # --- Determine items selected in OTHER rows --- (V2 Logic Reinstated)
    items_selected_elsewhere = set()
    for k in range(st.session_state.item_count):
        if i == k: continue # Skip the current row
        item_in_row_k = st.session_state.get(f"item_{k}")
        if item_in_row_k: items_selected_elsewhere.add(item_in_row_k)
    ...
    with col1:
        # Item selection - Uses FILTERED options list
        st.selectbox(
            label=f"Item {i}", # 0-based index
        ...
        # Note input
        st.text_input(
            label=f"Note {i}", key=f"note_{i}",
            placeholder="Special instructions...", label_visibility="collapsed"
        )
    with col2:
        ...
        # Quantity input
        st.number_input(
            label=f"Quantity {i}", min_value=1, step=1,
            key=f"qty_{i}", label_visibility="collapsed"
        )
    st.markdown("---")


# --- Pre-Submission Checks (NO summary display here) ---
# Calculate checks needed to enable/disable the submit button
items_data_for_check = []
item_names_processed_for_check = set()
has_duplicates_in_state = False
for i in range(st.session_state.item_count):
    item_name = st.session_state.get(f"item_{i}")
    item_qty = st.session_state.get(f"qty_{i}", 0)
    if item_name and item_qty > 0:
        if item_name in item_names_processed_for_check: has_duplicates_in_state = True
        item_names_processed_for_check.add(item_name)
        items_data_for_check.append(item_name)
has_valid_items = bool(items_data_for_check) # True if list is not empty
...
if has_duplicates_in_state:
    st.warning("Duplicate items detected. Please remove duplicates to enable submission.")
...
if st.button("Submit Indent Request", type="primary", use_container_width=True, disabled=submit_disabled, help=tooltip_message if submit_disabled else "Submit the current indent"):
    ...
    final_has_duplicates = False # Final rigorous check
    local_item_to_unit_lower = st.session_state.get('item_to_unit_lower', {})
    for i in range(st.session_state.item_count):
    ...
    if not items_to_submit_final: st.error("No valid, unique items to submit."); st.stop()
    if final_has_duplicates: st.error("Duplicates still detected. Submission aborted."); st.stop()
    ...
    try:
        ...
        for item, qty_val, unit, note_val in items_to_submit_final:
            rows_to_add.append([ mrn, timestamp, current_dept, formatted_date, item, str(qty_val), unit, note_val if note_val else "N/A" ])

        if rows_to_add:
            with st.spinner(f"Submitting indent {mrn} to Google Sheet..."):
                sheet.append_rows(rows_to_add, value_input_option='USER_ENTERED')

            # --- Display Success Message & POST-SUBMISSION SUMMARY ---
            st.success(f"Indent submitted successfully! MRN: {mrn}")
            st.balloons()
            st.markdown("---")
            st.subheader("Submitted Indent Summary")
            st.info(f"**MRN:** {mrn} | **Department:** {current_dept} | **Date Required:** {formatted_date}")
            submitted_df = pd.DataFrame(items_to_submit_final, columns=["Item", "Qty", "Unit", "Note"])
            st.dataframe(submitted_df, hide_index=True, use_container_width=True)
            total_submitted_qty = sum(item[1] for item in items_to_submit_final)
            st.markdown(f"**Total Submitted Quantity:** {total_submitted_qty}")
            st.markdown("---")
            # Pause to allow viewing summary before state reset/rerun
            ...
            # Option 1: Use stop() to pause - user needs to interact/refresh for next indent
            st.info("Please refresh the page or interact to start a new indent.")
            st.stop()
            # Option 2: Add a button to trigger rerun (more explicit)
