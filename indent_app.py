...
# This block ensures data is loaded ONCE per session and stored reliably in state
if 'master_item_list' not in st.session_state or 'item_to_unit_lower' not in st.session_state:
     ...
     # Populate session state explicitly
...
# Use the lists/maps from session state reliably from now on
# Use .get with default empty list/dict for safety in case state setting failed above
...
if not master_item_names:
    st.error("Item list is empty or could not be loaded into session state. Cannot proceed."); st.stop()
...
def update_unit_display(index):
    # ... (Callback function remains the same - reads map from state) ...
    selected_item = st.session_state.get(f"item_{index}")
    local_map = st.session_state.get('item_to_unit_lower', {}) # Read map from state
...
with col1_btn:
    if st.button("➕ Add Item"): # Using emoji for visual distinction
...
for i in range(st.session_state.item_count):
    ...
    with col1:
        st.selectbox( # Item selection
            label=f"Item {i}",
            options=[""] + master_item_names, # Use full list from state variable
            key=f"item_{i}",
    ...
    st.divider() # Use divider for cleaner look
...
if st.button("Submit Indent Request", type="primary", use_container_width=True, disabled=submit_disabled, help=tooltip_message if submit_disabled else "Submit the current indent"):
    ...
    try:
        ...
        if rows_to_add:
            ...
            # Re-initialize state for the single remaining row is needed if we don't fully clear and rely on setdefault at top
...
if 'submitted_data_for_summary' in st.session_state:
    ...
    # Removed st.stop() here - allows interaction below summary if needed, button reruns anyway.
