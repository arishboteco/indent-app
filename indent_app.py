...
def get_reference_data(_client):
    ...
    try:
        ...
        # st.exception(e) # Uncomment for detailed traceback if needed during dev
        return [], {}
...
if st.button("Submit Indent Request", type="primary", use_container_width=True, disabled=submit_disabled, help=tooltip_message if submit_disabled else "Submit the current indent"):
    ...
    try:
        ...
        if rows_to_add:
            ...
            st.rerun() # Rerun needed here to transition to showing the summary
...
if 'submitted_data_for_summary' in st.session_state:
    ...
    if st.button("Start New Indent"):
        ...
        st.rerun()
    # Note: Without st.stop(), the empty form elements might render briefly below the summary
    # until "Start New Indent" is clicked and the rerun happens. This is usually acceptable.
